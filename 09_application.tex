...
signal $\hat{G}_{i,t}$-- revealed excess-demand measure from the previous quarter's tatonnement---and
derives a required investment vector $\Delta\vec{K}_t$ via a
third-order Neumann truncation of the open Leontief capital-requirements
system:
\begin{equation}
  \Delta\vec{K}_t
    = \sum_{k=1}^{3} M^{k}
      \left[\hat{G}^{k}\vec{C}_{t-1} + g^{k} \vec{G}_{t}\right]
\end{equation}
where the sum is computed iteratively as a sequence of sparse matrix--vector
products over the normalised matrix $\bar{A}$.
Government expenditure $G_t$ grows exogenously at a fixed step $g_{\text{step}}
...
All random shocks are pre-drawn before the month loop to ensure
full reproducibility.
To bypass cross-language interpreter overhead between Python and Julia when orchestrating multi-agent systems, household preference objects are forcibly decoupled into natively typed Julia float blocks. Highly optimising these structures across memory-contiguous column-major layouts permits the tatonnement to be functionally flattened directly into strict BLAS native array multiplications entirely inside the intra-month iterative inner loop, clearing thousands of multi-household demand checks instantly.
...
\begin{align}\label{eq:dual_update}
  \ln \lambda_{K,j}^{(t+1)} &= \ln y_{K,j}^{(t)} + \eta_K g_{K,j}^{(t)} \notag \\
  \ln \lambda_{L}^{(t+1)} &= \ln y_{L}^{(t)} + \eta_L g_L^{(t)},
\end{align}
%
where $\eta_K \approx 0.4$ and $\eta_L \approx 0.5$ are the sectoral learning rates. 
...
Each iteration of~\eqref{eq:dual_update} requires exactly \textbf{two Matrix-Vector Products (MVPs)}: one forward Neumann application $M\vec{C}$ for capacity check, and one adjoint application $M^\top\vec{\lambda}_K$ for shadow price valuation.
The total computational cost per quarter is typically measured in MVPs, where successful convergence at $10^{-5}$ tolerance is achieved within \textbf{360--400 MVPs}.
...
The algorithm terminates successfully if the normalized constraint violations disappear and the complementary slackness conditions hold at the true iterate $\vec{\lambda}^{(t)}$ at a high-precision threshold:
$$
\lambda_{L}^{(t)} |s_L| \leq 10^{-5} ,\  
\lambda_{K,j}^{(t)}|s_{K,j}| \leq 10^{-5}
$$
...
for larger IO benchmarks. The solver convergence is robust, typically requiring \textbf{180--200 iterations} (equivalent to \textbf{360--400 MVPs}) to satisfy the strict $10^{-5}$ KKT tolerance.
