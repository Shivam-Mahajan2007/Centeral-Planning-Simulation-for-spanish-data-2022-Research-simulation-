"""
plots.py — Publication-quality diagnostics for the cybernetic planning simulation.

All monetary values are in EUR; plots display Billions EUR (÷ 1e9).
Annualised figures multiply quarterly values by 4.
Each function writes both .png and .pdf to the supplied path.
"""
...
def plot_gdp(history, save_path, P_initial=None, A=None, real_scale_factor=None):
    ...
    ax.plot(ts, gdp, marker="o", label="Real GDP (annualised)")
...
def plot_output_consumption(history, groups, save_path, P_0=None):
    ...
    def valued_grps(key):
        prices = P_0 if P_0 is not None else history[0]["P"]
        return {g: np.array([(h[key] * prices)[idx].sum() for h in history])
                for g, idx in groups.items()}
    ...
    _xticks(ax1, ts, ql); ax1.legend(); ax1.grid()
    ...
    _xticks(ax2, ts, ql); ax2.legend(); ax2.grid()
    savefig(fig, save_path)
...
def plot_investment(history, groups, save_path, P_0=None):
    ...
    prices = P_0 if P_0 is not None else history[0]["P"]
    I_val_grps = {
        g: np.array([(h["I_vec"] * prices)[idx].sum() for h in history])
        for g, idx in groups.items()
    }
    fig, ax = plt.subplots(figsize=(12, 7))
    bot = np.zeros(T)
    ...
    _xticks(ax, ts, ql); ax.legend(); ax.grid()
    savefig(fig, save_path)
...
def plot_capital(history, groups, save_path, P_0=None):
    ...
    ax.set_title("Capital Stock by Sector Group (Q1 prices)")
    _xticks(ax, ts, ql); ax.legend(); ax.grid()
    savefig(fig, save_path)
...
def plot_labor_utilization(history, save_path):
    ...
    ax.set_title("Aggregate Labor Utilization")
    _xticks(ax, ts, ql); ax.legend(loc="lower right"); ax.grid(alpha=0.3)
    savefig(fig, save_path)
...
def plot_shadow_price_index(history, save_path):
    ...
    ax.set_title("System-Wide Cost Level")
...
def plot_cybernetic_signals(history, save_path):
    ...
    ax1.set_ylabel("Price Drift (%)", color="#8e44ad")
    ax1.plot(ts, drift, marker="o", color="#8e44ad", label="Price Drift")
    ax1.tick_params(axis='y', labelcolor="#8e44ad")
    ax2 = ax1.twinx()
    ax2.set_ylabel("Nominal Excess Demand (B EUR)", color="#d35400")
    ax2.plot(ts, ed, marker="s", linestyle="--", color="#d35400", label="Nominal Excess Demand")
    ax2.tick_params(axis='y', labelcolor="#d35400")
    ax1.set_title("Cybernetic Feedback Signals")
    _xticks(ax1, ts, ql); fig.tight_layout(); ax1.grid(alpha=0.2)
    savefig(fig, save_path)
...
def plot_labor_productivity(history, save_path):
    ...
    ax.set_title("Aggregate Labor Productivity (Real GDP / Labor Hour)")
    _xticks(ax, ts, ql); ax.grid(alpha=0.3)
    savefig(fig, save_path)
...
def plot_excess_demand(history, save_path):
    ...
    for h in history:
        ...
        nominal_ad = max(np.sum(p_vec * (c_star + i_vec + g_vec)), 1e-30)
        m          = p_vec * (c_star - demand) / nominal_ad
    ...
    ax.set_title("Value-Weighted Excess Demand  P·(C* − D) / AD")
    _xticks(ax, ts, ql); ax.grid(alpha=0.3)
    savefig(fig, save_path)
...
def plot_inflation(history, save_path):
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(ts, inf, marker="o", color="#e67e22", linewidth=2.5, label="Aggregate Inflation (Laspeyres)")
    ax.axhline(0, color="black", linestyle="--", alpha=0.5)
    ax.set_ylabel("Quarterly Inflation Rate (%)")
    ax.set_title("Price Stability (Aggregate Inflation)")
    _xticks(ax, ts, ql); ax.legend(); ax.grid(alpha=0.3)
    savefig(fig, save_path)
...
def plot_investment_gdp_ratio(history, save_path, ref_pct: float = 19.8,
                              start_year: int = 2022):
    """Plot quarterly I/GDP with annual-average bars and 5-year rolling average."""
    import matplotlib.pyplot as plt
    T  = len(history)
    ts = np.arange(T)
    ...
    roll5_xs = bar_xs[:]
    roll5_ys = [float(np.mean(bar_hs[max(0, i - 4): i + 1])) for i in range(len(bar_hs))]

    fig, ax = plt.subplots(figsize=(13, 7))
    ax.bar(bar_xs, bar_hs, width=3.5, color="#3498db", alpha=0.28, zorder=1, label="Annual Average")
    for bx, bh, bl in zip(bar_xs, bar_hs, bar_lbls):
        ax.text(bx, bh + 0.15, f"{bh:.1f}%", ha="center", va="bottom", fontsize=8, color="#2980b9")
    ax.plot(ts, i_pct, marker="o", markersize=4, linewidth=2, color="#2c3e50", zorder=3,
            label="Quarterly I / GDP (%)")
    ax.plot(roll5_xs, roll5_ys, marker="D", markersize=6, linewidth=2.5, linestyle="--",
            color="#e67e22", zorder=4, label="5-Year Rolling Average")
    ax.axhline(ref_pct, color="#c0392b", linestyle=":", linewidth=1.5, alpha=0.75,
               label=f"Reference {ref_pct:.0f}% (Spain hist. avg)")
    ax.set_ylabel("Investment / GDP (%)")
    ax.set_title("Gross Investment as % of Real GDP  —  Quarterly & Annual Averages")
    _xticks(ax, ts, ql); ax.legend(loc="upper right"); ax.grid(alpha=0.25)
    fig.tight_layout()
    savefig(fig, save_path)


def plot_firm_income_distribution(hist, n, out_path):
    """Histogram of aggregate income across firms (units in absolute EUR)."""
    import matplotlib.pyplot as plt
    h_final = hist[-1]
    v_MIP   = h_final.get("v_MIP", np.zeros(n))
    X_f     = h_final.get("X_f",   np.zeros((n, 2)))
    
    # Calculate aggregate income for each firm (sum across sectors)
    # X_f shape: (n_sectors, n_firms)
    firm_incomes = np.sum(v_MIP[:, None] * X_f, axis=0)
    
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.hist(firm_incomes, bins=25, color="#2c3e50", alpha=0.8, edgecolor='white')
    ax.set_title("Distribution of Firm-Level Aggregate Income", fontsize=14, fontweight='bold')
    ax.set_xlabel("Quarterly Income (EUR)")
