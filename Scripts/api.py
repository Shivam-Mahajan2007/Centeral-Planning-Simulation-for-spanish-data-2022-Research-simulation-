...
Runs the simulation in a background thread and streams per-quarter progress
to the browser via Server-Sent Events. Charts generated by plots.py are served
...
# ── Path setup so imports resolve from the Scripts directory ──────────────────
...
def _run_simulation_thread(config: dict):
    ...
    try:
        ...
        from data_loader import sector_groups
